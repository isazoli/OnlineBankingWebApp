...
public class TransferMoneySubmitController {
	...
	public ModelAndView transferMoneySubmit(
		...
		if (sourceAccount.getId().equals(targetAccount.getId()))
			return error("transfer.money.account.same", transferRequest, redirectAttributes, bindingResult);
		// validation error: source and target accounts are in a different currency
		if (!sourceAccount.getCurrency().equals(targetAccount.getCurrency()))
			return error("transfer.money.account.not.same.currency", transferRequest, redirectAttributes, bindingResult);
		final String amountStr = transferRequest.getAmount();
		try {
			final Long amount = Long.valueOf(amountStr);
			// validation error: not positive
			if (amount <= 0)
				return error("transfer.money.amount.not.positive", transferRequest, redirectAttributes, bindingResult);
			mv.addObject("amount", amountStr);
		} catch (NumberFormatException e) {
			// validation error: not a number
			return error("transfer.money.amount.not.number", transferRequest, redirectAttributes, bindingResult);
		}
